...
const AddCaseStudyForm = () => {
  ...
  useEffect(() => {
    if (editId) {
      // Load existing case study data for editing
      const loadCaseStudyData = async () => {
        try {
          const res = await fetch('/api/case-studies');
          if (res.ok) {
            const caseStudiesData = await res.json();
            const caseStudy = caseStudiesData.caseStudies.find((c: any) => c.id === editId);
            if (caseStudy) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && editId) {
        ...
        const caseStudyIndex = caseStudiesData.caseStudies.findIndex((c: any) => c.id === editId);
        if (caseStudyIndex !== -1) {
          caseStudiesData.caseStudies[caseStudyIndex] = formData;
        }
      } else {
        // Add new case study
      ...
      if (updateRes.ok) {
        setNotification({ type: 'success', message: `Case study ${isEditing ? 'updated' : 'added'} successfully!` });
        setTimeout(() => router.push('/admin/case-studies'), 2000);
      } else {
        setNotification({ type: 'error', message: `Error ${isEditing ? 'updating' : 'adding'} case study. Please try again.` });
      }
    } catch (error) {
      setNotification({ type: 'error', message: `Error ${isEditing ? 'updating' : 'adding'} case study. Please try again.` });
    }
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600">
              {isEditing ? 'Update the case study details' : 'Add a new case study to showcase your work'}
            </p>
          </div>
          <Button
